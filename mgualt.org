...
**** Option 4
    ...
    what I use, but it requires you compile a custom version of Emacs
    provided by Mitsuharu.)

#+source: fullscreen-4
#+begin_src emacs-lisp 
  (defun fullscreen (&optional
                     f) (interactive) (set-frame-parameter f
                     'fullscreen (if (frame-parameter f
                     'fullscreen) nil
                     'fullboth))) (global-set-key (kbd "C-c f")
                     'fullscreen)
  ;; no need to fulscreen every new frame
...
Polymode is a new package that supports multiple modes inside a single
...
#+begin_src emacs-lisp 
  ...
  (setq dired-listing-switches "-gto")
...
** Deft mode [[http://jblevins.org/projects/deft/][Deft project]]
#+srcname: final-custom
#+begin_src emacs-lisp
  (require 'deft)
  (setq deft-extension "org")
  (setq deft-directory "~/Dropbox/Research/Notes/deft/")
...
#+srcname: Colour
#+begin_src emacs-lisp
  (load-theme 'mgualt-solarized-dark t)
#+end_src
...
#+begin_src emacs-lisp
    ...
    ;; scroll one line at a time (less "jumpy" than defaults)
      (setq mouse-wheel-scroll-amount '(1 ((shift) . 0.2))) ;; one line at a time
    ...
    ;; For SyncTeX to work with Skim, the full path to emacsclient must be in Skim
  ...
  (defun run-latexmk ()
    ...
    (let ((TeX-save-query nil)
      ...
      (TeX-save-document "") ;; this works
      (TeX-run-TeX "latexmk" ;; this command does work. 
  ...
  ;; not needed since happens in init.el
