...
*** PATH settings
Emacs doesn't seem to see the system path which is set in .bash_profile, 
...
#+RESULTS:
| /usr/local/bin | /usr/texbin | /Users/mgualt/.rvm/gems/ruby-1.9.2-p320/bin/ | /Users/mgualt/.rvm/gems/ruby-1.9.2-p320@global/bin/ | /Users/mgualt/.rvm/rubies/ruby-1.9.2-p320/bin/ | /Users/mgualt/.rvm/bin/ | /usr/local/Cellar/gettext/0.18.1.1/include/ | /usr/gcc-4.7.1/bin/ | /usr/local/bin/ | /usr/bin/ | /bin/ | /usr/sbin/ | /sbin/ | /usr/local/bin/ | /opt/X11/bin/ | /usr/X11/bin/ | /usr/texbin/ |

Is this necessary? it seems so. 
#+srcname: path-settings-PATH
#+begin_src emacs-lisp
(setenv "PATH"
  (concat
   "/usr/local/bin" ":"
...
#+RESULTS:
: /usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/local/bin:/usr/texbin:/Users/mgualt/.rvm/gems/ruby-1.9.2-p320/bin:/Users/mgualt/.rvm/gems/ruby-1.9.2-p320@global/bin:/Users/mgualt/.rvm/rubies/ruby-1.9.2-p320/bin:/Users/mgualt/.rvm/bin:/usr/local/Cellar/gettext/0.18.1.1/include:/usr/gcc-4.7.1/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin:/opt/X11/bin:/usr/X11/bin:/usr/texbin
...
#+end_src
*** Frame position
Set the position of the frame and define function arrange-frame  
#+srcname: arrange-frame
#+begin_src emacs-lisp 
...
Not sure what is supposed to be commented out here. Resources from [[http://sourceforge.net/apps/mediawiki/skim-app/index.php?title=TeX_and_PDF_Synchronization#Considerations_for_Emacs][sourceforge]].
...
#+begin_src emacs-lisp :tangle no    
    ...
    '("/Users/mgualt/Dropbox/Research/Bibliography"))

    ;; Default bibliography
    (setq reftex-default-bibliography
    '("/Users/mgualt/Dropbox/Research/Bibliography/Master.bib"))
...
*** Outline mode
turning on the outline minor mode,
opening with body hidden, 
and using easy bindings for navigation.

#+srcname: outline-minor-mode
#+begin_src emacs-lisp
  (add-hook 'TeX-mode-hook '(lambda () (outline-minor-mode 1)))
  (add-hook 'outline-minor-mode-hook '(lambda () (hide-body)))
  (add-hook 'outline-minor-mode-hook '(lambda () (require 'outline-mode-easy-bindings)))
#+end_src

*** Wrapping
Following [[http://www.math.ntnu.no/~stacey/HowDidIDoThat/LaTeX/versioning.html][Andrew Stacey]]'s advice, turn on longlines-mode

#+srcname: longlines-mode
#+begin_src emacs-lisp
  (add-hook 'tex-mode-hook '(lambda () (longlines-mode 1)))
#+end_src

*** Cosmetics
Stop Auctex from using different fonts for the sections and subscripts.
...
#+begin_src emacs-lisp :tangle no
 (setenv "PATH" (concat "/Users/mgualt/.cabal/bin:" (getenv "PATH")))
 (load "pandoc-mode")
...
section. The required style files used below are available at
https://github.com/kjhealy/latex-custom-kjh. You may need to adjust or
remove some of these settings depending on your preferences and local
configuration.
...
*** Cosmetic Settings
Startup using [[http://orgmode.org/manual/Clean-view.html][org-indent-mode]].

#+source: org-indent-mode
#+begin_src emacs-lisp
...
** Initial Buffer
Make the initial buffer be the main directory for all my projects.

#+srcname: initial-buffer-choice
#+begin_src emacs-lisp 
